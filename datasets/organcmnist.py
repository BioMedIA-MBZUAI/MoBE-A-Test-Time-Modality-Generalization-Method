...
template = [
    "a coronal CT slice of the {}.",
    "a coronal abdominal CT image of the {}.",
]
...
class OrganCMNISTDataset(TorchDataset):
...
class OrganCMNIST:
    ...
    def __init__(self, root, npz_filename=None):
        ...
        self.test = OrganCMNISTDataset(images, labels, transform=None)
    ...
    def _resolve_npz_path(root, npz_filename=None):
        if npz_filename:
            candidates = [
                os.path.join(root, "organcmnist", npz_filename),
                os.path.join(root, npz_filename),
            ]
        else:
            candidates = [
                os.path.join(root, "organcmnist", "organcmnist.npz"),
                os.path.join(root, "organcmnist.npz"),
            ]
        for candidate in candidates:
            if os.path.isfile(candidate):
                return candidate
        target = npz_filename or "organcmnist.npz"
